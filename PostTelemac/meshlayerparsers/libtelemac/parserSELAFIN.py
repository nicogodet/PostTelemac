...
def getValueHistorySLF(hook, tags, time, support, NVAR, NPOIN3, NPLAN, varsIndexes, varsName):
    ...
    if fsize == 4:
        z = np.zeros((len(varsIndexes), lens, len(time)), dtype=np.float32)
    else:
        z = np.zeros((len(varsIndexes), lens, len(time)), dtype=np.float64)
                    ...
                    if type(xy) == type(()):  # xp is a pair (x,y) and you need interpolation
                        for plan in zp:  # /!\ only list of plans is allowed for now
                            ...
                            for inod in range(len(bn)):
                                z[jvar][ipt][it] += bn[inod] * VARSOR[ln[inod] + plan * NPOIN3 / NPLAN]
...
def getValuePolylineSLF(hook, tags, time, support, NVAR, NPOIN3, NPLAN, varsIndexes, varsName):
    ...
    # ~~ Extract time profiles ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if fsize == 4:
        z = np.zeros((len(varsIndexes), len(time), lens, len(support)), dtype=np.float32)
    else:
    ...
    for it in range(len(time)):  # it is from 0 to len(time)-1
        ...
        for ivar in range(NVAR):  # ivar is from 0 to NVAR-1
            ...
            if ivar in varsIndexes:  # extraction of a particular variable
                ...
                for ipt in range(
                    len(support)
                ):  # ipt is from 0 to lens-1 (= all points extracted and all plans extracted)
                    xy, zp = support[ipt]
                    for ipl in range(len(zp)):  # /!\ only list of plans is allowed for now
                        z[varsIndexes.index(ivar)][it][ipl][ipt] = 0.0
                        ln, bn = xy
                        for inod in range(len(bn)):
                            z[varsIndexes.index(ivar)][it][ipl][ipt] += (
                                bn[inod] * VARSOR[ln[inod] + zp[ipl] * NPOIN3 / NPLAN]
                            )
            else:
...
def getValuePolyplanSLF(hook, tags, time, support, NVAR, NPOIN3, NPLAN, varsIndexes, varsName):
    ...
    # ~~ Extract planes ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    if fsize == 4:
        z = np.zeros((len(varsIndexes), len(time), len(support), NPOIN3 / NPLAN), dtype=np.float32)
    else:
        z = np.zeros((len(varsIndexes), len(time), len(support), NPOIN3 / NPLAN), dtype=np.float64)
        ...
        for ivar in range(NVAR):  # ivar is from 0 to NVAR-1
            ...
            if ivar in varsIndexes:  # extraction of a particular variable
                ...
                for ipl in range(len(support)):  # ipt is from 0 to len(support) (= all plans extracted)
                    z[varsIndexes.index(ivar)][it][ipl] = VARSOR[
                        support[ipl] * NPOIN3 / NPLAN : (support[ipl] + 1) * NPOIN3 / NPLAN
                    ]
            else:
                f.read(fsize * NPOIN3)  # the file pointer advances through all records to keep on track
            f.read(4)
...
class CONLIM:
    ...
    def setNUMLIQ(self, closedContours):
        ...
        for contour in closedContours:  # for the domain boundary and eavery islands
            ...
            inode = 0
            while not solids[self.KFRGL[contour[inode]]] and inode < len(contour):
                inode += 1
...
class SELAFIN:
   ...
   """
    ...
    def getVariablesAt(self, frame, varsIndexes):
        ...
        ftype, fsize = self.file["float"]
        if fsize == 4:
            z = np.zeros((len(varsIndexes), self.NPOIN3), dtype=np.float32)
        else:
            z = np.zeros((len(varsIndexes), self.NPOIN3), dtype=np.float64)
    ...
    def getVALUES(self, t):
        ...
        for v in self.alterZnames:
            for iv in range(len(self.VARNAMES)):
                if v.lower() in self.VARNAMES[iv].lower():
                    VARSOR[iv] = self.alterZm * VARSOR[iv] + self.alterZp
            for iv in range(len(self.CLDNAMES)):
                if v.lower() in self.CLDNAMES[iv].lower():
                    VARSOR[iv + self.NBV1] = self.alterZm * VARSOR[iv + self.NBV1] + self.alterZp
        return VARSOR
    ...
    def getSERIES(self, nodes, varsIndexes=[], showbar=True):
        ...
        try:
            ...
            temp = list(zip(range(len(nodes)), nodes))
            # temp1 = temp[:]
            temp2 = list(temp)
            onodes = np.sort(np.array(temp2, dtype=[("0", int), ("1", int)]), order="1")

        # ~~ Extract time profiles ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        if fsize == 4:
            z = np.zeros((len(varsIndexes), len(nodes), len(self.tags["cores"])), dtype=np.float32)
        else:
            z = np.zeros((len(varsIndexes), len(nodes), len(self.tags["cores"])), dtype=np.float64)
...
class SELAFINS:
    ...
    def isSuite(self):
        ...
        for slf in self.slfs[1:]:
            for v in slf.VARNAMES:
                same = same and (v in self.slfs[0].VARNAMES)
            for v in slf.CLDNAMES:
                same = same and (v in self.slfs[0].CLDNAMES)
        return same

    def isMerge(self):
        same = True
        for slf in self.slfs[1:]:
            mmax = 1.0e-5 + np.max(slf.tags["times"]) + np.max(self.slfs[0].tags["times"])
            ...
            if len(slf.tags["times"]) != len(self.slfs[0].tags["times"]):
                ...
                same = same and (
                    accuracy
                    > np.max(slf.tags["times"] - self.slfs[0].tags["times"])
                    - np.min(slf.tags["times"] - self.slfs[0].tags["times"])
                )
        return same
    ...
    def putContent(self, fileName):  # TODO: files also have to have the same header
        if self.suite and self.merge:
            if len(self.slfs) == 2:  # /!\ difference only between two files
                ...
                self.slf.putContent(fileName)  # just a copy
        elif self.suite:
            self.slf.fole.update({"hook": open(fileName, "wb")})
            self.slf.fole["name"] = fileName
            ibar = 0  # ; pbar = ProgressBar(maxval=len(self.slf.tags['times'])).start()
...
class PARAFINS(SELAFINS):
    def __init__(self, fileName, root=None):
        ...
        if root != None:
            ...
            for i, slf in zip(range(len(self.slfs)), self.slfs):
                self.mapPOIN[slf.IPOB3 - 1] = i

    def alterEndian(self):
        self.slf.alterEndian()
    ...
    def getPALUES(self, t):
        ...
        if len(self.slfs) > 0:
            if fsize == 4:
                VARSOR = np.zeros((self.slf.NBV1 + self.slf.NBV2, self.slf.NPOIN3), dtype=np.float32)
            else:
                VARSOR = np.zeros((self.slf.NBV1 + self.slf.NBV2, self.slf.NPOIN3), dtype=np.float64)
            for slf in self.slfs:
                VARSUB = slf.getVALUES(t)
                for v in range(self.slf.NVAR):
                    VARSOR[v][slf.IPOB3 - 1] = VARSUB[v]
        else:
    ...
    def getSERIES(self, nodes, varsIndexes=[]):
        ftype, fsize = self.file["float"]
        if varsIndexes == []:
            varsIndexes = self.slf.VARINDEX
        if len(self.slfs) > 0:
            if fsize == 4:
                z = np.zeros((len(varsIndexes), len(nodes), len(self.slf.tags["cores"])), dtype=np.float32)
            else:
                z = np.zeros((len(varsIndexes), len(nodes), len(self.slf.tags["cores"])), dtype=np.float64)
            ...
            for islf, slf in zip(range(len(self.slfs)), self.slfs):
                ...
                subGnodes = np.compress(
                    mproc == islf, np.array(zip(range(len(nodes)), nodes), dtype=[("r", int), ("n", int)])
                )
                subLnodes = (
                    np.searchsorted(np.sort(slf.IPOB2), subGnodes["n"]) + 1
                ...
                for v in range(len(varsIndexes)):
                    z[v][subGnodes["r"]] = subz[v]
            return z
        else:
            return self.slf.getSERIES(nodes)  # ,varsIndexes not necessary
        ...
        for slf in self.slfs:  # TODO: do this loop in python parallel with a bottle neck at islf.getVALUES(t)
            ...
            if self.slf.NPOIN2 == islf.NPOIN2:
                ...
                if islf.NPLAN > 1:
                    ...
                    sub.IPOB3 = np.ravel(
                        np.add(
                            np.repeat(sub.IPOB2, islf.NPLAN).reshape((sub.NPOIN2, islf.NPLAN)),
                            sub.NPOIN2 * np.arange(islf.NPLAN),
                        ).T
                    )
                    sub.IKLE3 = np.repeat(sub.NPOIN2 * np.arange(islf.NPLAN - 1), sub.NELEM2 * islf.NDP3).reshape(
                        (sub.NELEM2 * (islf.NPLAN - 1), islf.NDP3)
                    ) + np.tile(
                        np.add(np.tile(sub.IKLE2, 2), np.repeat(sub.NPOIN2 * np.arange(2), sub.NDP2)),
                        (islf.NPLAN - 1, 1),
                    )
                else:
            ...
            # ~~> filtered partitions / non reversable !
                ...
                GKLE2 = islf.IKLE2[
                    np.where(
                        np.sum(np.in1d(islf.IKLE2, np.sort(indices)).reshape(islf.NELEM2, islf.NDP2), axis=1)
                        == islf.NDP2
                    )
                ]
                ...
                for k in range(len(GKLE2)):
                    for ki in range(islf.NDP2):
                        sub.IKLE2[k][ki] = KNOGL[
                            GKLE2[k][ki]
                        ]  # /!\ sub.IKLE2 has a local numbering, fit to the boundary elements
            ...
            for t in range(len(islf.tags["times"])):
                ...
                for ivar, var in zip(range(sub.NVAR), islf.getVALUES(t)):
                    for n in range(sub.NPLAN):
                        VARSORS[ivar][n * sub.NPOIN2 : (n + 1) * sub.NPOIN2] = var[
                            n * islf.NPOIN2 : (n + 1) * islf.NPOIN2
                        ][indices]
